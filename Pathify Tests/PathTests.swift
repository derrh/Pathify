...
class PathTests: XCTestCase {
    ...
    func test_parameterized_paths() {
        ...
        XCTAssertEqual(slashInt.match("/12") ?? Int.max, 12, "Should find the 12")
        
        let int = path(AnyInt)
        XCTAssertEqual(int.path(33), "33", "single component")
        XCTAssertEqual(int.match("33") ?? Int.max, 33, "just the number 33, everyone")
        
        let slashIntSlashString = slashInt/AnyString
        XCTAssertEqual(slashIntSlashString.path(3, "foo"), "/3/foo", "int and a string path")
        XCTAssertEqual(slashIntSlashString.match("/9/foo")?.0 ?? 999, 9, "capture 1st parameter")
        XCTAssertEqual(slashIntSlashString.match("/9/foo")?.1 ?? "cats", "foo", "capture 2nd parameter")
        ...
        XCTAssertEqual(addStaticComponent.match("/1233/bar") ?? Int.max, 1233, "matches the first path componet")
    }
    ...
    func test_3() {
        ...
        XCTAssertEqual(path.match("3/5/14")?.0 ?? Int.max, 3, "first component should be 3")
        XCTAssertEqual(path.match("3/5/14")?.1 ?? Int.max, 5, "first component should be 3")
        XCTAssertEqual(path.match("3/5/14")?.2 ?? Int.max, 14, "first component should be 3")
    }
    ...
    func test_4() {
        ...
        XCTAssertEqual(path.match("/1/2/3/9")?.0 ?? Int.max, 1, "first component should be 3")
        XCTAssertEqual(path.match("/1/2/3/9")?.1 ?? Int.max, 2, "first component should be 3")
        XCTAssertEqual(path.match("/1/2/3/9")?.2 ?? Int.max, 3, "first component should be 3")
        XCTAssertEqual(path.match("/1/2/3/9")?.3 ?? Int.max, 9, "first component should be 3") // ya I copy/paste... so what?
    }
    ...
    func test_5() {
        ...
        XCTAssertEqual(path.match("1/2/3/4/88")?.0 ?? Int.max, 1, "component should match")
        XCTAssertEqual(path.match("1/2/3/4/88")?.1 ?? Int.max, 2, "component should match")
        XCTAssertEqual(path.match("1/2/3/4/88")?.2 ?? Int.max, 3, "component should match")
        XCTAssertEqual(path.match("1/2/3/4/88")?.3 ?? Int.max, 4, "component should match")
        XCTAssertEqual(path.match("1/2/3/4/88")?.4 ?? Int.max, 88, "component should match")
    }
}
